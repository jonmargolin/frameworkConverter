...
export type PromptMethod = (origin: string, target: string) => string;
...
`,
  ...
  UNIFY_TEMPLATE_LOGIC = 'Merge the logic (ts) and template (html) files into one unified component.',
  KEEP_CSS_EXTENSION = 'css or css based files shouldnt be changed content or name wise',
  ADD_COMPONENT_SUFFIX = "Add '.component' to the filename, so 'example' becomes 'example.component'.",
  REMOVE_COMPONENT_SUFFIX = "Transform the Angular component filenames from kebab-case with '.component' suffix (e.g., 'app.component') to React's PascalCase naming convention (e.g., 'AppComponent')",
...
export const promptMethods: Record<string, PromptMethod> = {
